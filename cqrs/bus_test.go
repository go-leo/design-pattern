...
func TestBus(t *testing.T) {
	...
	errC = bus.AsyncExec(context.Background(), examCmd{})
	assert.ErrorIs(t, <-errC, errNotPassed)
	...
	resC, errC := bus.AsyncQuery(context.Background(), &studentQuery{Id: 11})
	assert.NoError(t, <-errC)
	assert.EqualValues(t, &studentResult{Name: "jax"}, <-resC)

	resC, errC = bus.AsyncQuery(context.Background(), teacherQuery{Id: 1})
	assert.ErrorIs(t, <-errC, errNotFoundTeacher)
}
